...
HAL_StatusTypeDef HTS221_ReadTemperature(HTS221_HandleTypeDef* hhts221)
{
	...
	uint16_t raw_temp = (int16_t)(data[1] << 8 | data[0]);
...
HAL_StatusTypeDef HTS221_ReadHumidity(HTS221_HandleTypeDef* hhts221)
{
	...
	if (HTS221_ReadRegisters(hhts221, HTS221_TEMP_OUT_L, data, 2) != HAL_OK) return HAL_ERROR;
	uint16_t raw_hum = (int16_t)(data[1] << 8 | data[0]);

	// Interpolate humidity using formula (1) from the Technical Note [TN1218, p. 2]
	uint32_t tmp32 = ((int32_t)(raw_hum - hhts221->cal.H1_T0_out)) * ((int32_t)(hhts221->cal.H1_rH - hhts221->cal.H0_rH));
	hhts221->humidity = (tmp32 / (hhts221->cal.H1_T0_out - hhts221->cal.H0_T0_out) + hhts221->cal.H0_rH);

	if (hhts221->humidity > 100) hhts221->humidity = 100; // Clamp to 100%
