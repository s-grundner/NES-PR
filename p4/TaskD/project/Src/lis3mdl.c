...
// Read / Write Registers with DMA

HAL_StatusTypeDef LIS3MDL_ReadRegisters_DMA(LIS3MDL_HandleTypeDef *hlis3mdl, uint8_t reg, uint8_t *data, uint16_t size)
{
	return HAL_I2C_Mem_Read_DMA(hlis3mdl->hi2c, hlis3mdl->address, reg, I2C_MEMADD_SIZE_8BIT, data, size);
}
...
// Read / Write Specific Registers with DMA

HAL_StatusTypeDef LIS3MDL_ReadXYZ_DMA(LIS3MDL_HandleTypeDef *hlis3mdl)
{
	static uint8_t data[6]; // needs to be preserved to be used from the callback
	if (LIS3MDL_ReadRegisters_DMA(hlis3mdl, LIS3MDL_OUT_X_L, data, 6) != HAL_OK)
		return HAL_ERROR;
